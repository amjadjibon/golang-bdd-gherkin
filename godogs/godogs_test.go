...
import (
	...
	"github.com/amjadjibon/golang-bdd-gherkin/godogs"
...
func thenIBuy(ctx context.Context, num int) error {
	godogsFromContext(ctx).Buy(num)
	return nil
}

func iAskTheWizardToDoubleThem(ctx context.Context) error {
	godogsFromContext(ctx).DoubleIfEven()
	return nil
}
...
func InitializeScenario(ctx *godog.ScenarioContext) {
	...
	ctx.Step(`^Then I buy (\d+)$`, thenIBuy)
	ctx.Step(`^I ask the wizard to double them$`, iAskTheWizardToDoubleThem)
	ctx.Step(`^I should have (\d+) godogs$`, thereShouldBeRemaining)
}
